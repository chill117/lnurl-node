	...
	validate: function(params) {
		try {
			...
			if (!_.isUndefined(commentAllowed)) {
				...
				if (commentAllowed.isGreaterThan(500)) {
					throw new HttpError('"commentAllowed" must be less than or equal to 500', 400);
				}
			}
	...
	info: function(secret, params) {
		...
		try {
			...
			return this.executeHook('payRequest:info', secret, params).then(() => {
				...
				const info = _.chain(params).pick('minSendable', 'maxSendable', 'metadata', 'commentAllowed').extend({
	...
	action: function(secret, params) {
		...
		try {
			...
			if (!_.isUndefined(comment)) {
				...
				if (commentAllowed > 0 && comment.length > commentAllowed) {
