			...
			checkSignature: (req, res, next) => {
				return Promise.resolve().then(() => {
					...
					return this.getApiKey(id).then(apiKey => {
						...
						assert.ok(isValidSignedQuery(query, key), new HttpError('Invalid API key signature', 403));
					});
...
Server.prototype.prepareLightningBackend = function(options) {
	assert.ok(options, 'Missing required argument: "options"');
	assert.strictEqual(typeof options, 'object', 'Invalid argument ("options"): Object expected');
	let Backend;
	...
	} else if (typeof options.backend === 'object') {
		assert.ok(options.backend.path, new Error('Invalid option ("lightning.backend"): Missing required property "path"'));
...
Server.prototype.registerLightningBackend = function(name, filePathOrPrototype) {
	...
	assert.strictEqual(typeof name, 'string', 'Invalid argument ("name"): String expected');
	assert.ok(filePathOrPrototype, 'Missing required argument: "filePathOrPrototype"');
	assert.ok(typeof filePathOrPrototype === 'string' || typeof filePathOrPrototype === 'function', 'Invalid argument ("filePathOrPrototype"): String or function expected');
	assert.ok(!this.RegisteredLightningBackends[name], `Cannot register Lightning Backend ("${name}"): Already registered`);
	let Prototype;
	if (typeof filePathOrPrototype === 'string') {
		Prototype = require(filePathOrPrototype);
	} else {
		Prototype = filePathOrPrototype;
	}
	// Check for missing required prototype methods.
	try { new Prototype; } catch (error) {
