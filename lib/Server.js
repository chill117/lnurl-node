...
Server.prototype.createWebServer = function(options) {
	...
	const { endpoint, host, port, protocol } = options;
	if (protocol !== 'http') {
		throw new Error(`Unknown or unsupported protocol: "${protocol}"`);
	}
	const app = express();
...
Server.prototype.middleware = function() {
	return {
		...
		signedLnurl: {
			...
			afterCheckSignature: (req, res, next) => {
				return this.executeHook('middleware:signedLnurl:afterCheckSignature', req, res).then(() => {
					next();
				}).catch(next);
			},
...
Server.prototype.prepareHooks = function() {
	...
	return _.chain([
		'login',
		'middleware:signedLnurl:afterCheckSignature',
	].concat(subprotocolHookNames)).map(name => {
...
Server.prototype.close = function() {
	...
	return this.store.close().then(() => {
		if (this.app && this.app.webServer) {
			return this.app.webServer.close(() => {
