	...
	return {
		...
		hook: {
			'error': (error, req, res, next) => {
				return this.executeHook('error', error, req, res).then(() => next(error)).catch(next);
			},
			'status': (req, res, next) => {
...
Server.prototype.prepareHook = function(name) {
	...
	return function() {
		return Promise.resolve().then(() => {
			...
			const promiseFactories = callbacks.map(callback => {
				return function() {
					return new Promise((resolve, reject) => {
...
Server.prototype.clearCallbacksBoundToHook = function(name) {
	this.assertHookExists(name);
	this.hooks[name].callbacks = [];
};
